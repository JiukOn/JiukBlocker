...
function resetDOM() {
  document.querySelectorAll('[data-e2e="recommend-list-item-container"], .video-feed-item, div[class*="DivItemContainer"]').forEach(el => {
...
function processElement(el) {
  ...
  if (blockAdsSpam && (el.querySelector('[data-e2e="sponsored-label"]') || el.innerText.toLowerCase().includes('sponsored'))) {
    el.style.display = 'none';
    return;
  }

  const descEl = el.querySelector('[data-e2e="video-desc"]');
  const userEl = el.querySelector('[data-e2e="video-author-uniqueid"]') || el.querySelector('[data-e2e="video-author-nickname"]');
  const text = `${descEl ? descEl.innerText : ''} ${userEl ? userEl.innerText : ''}`.trim();
...
function processDOM() {
  document.querySelectorAll('[data-e2e="recommend-list-item-container"], .video-feed-item, div[class*="DivItemContainer"]').forEach(processElement);
}
